    ...
    while (current) {
        if (current->productId == productId) {
            ...
            if (current->quantity <= 0) {
                ...
                // Push the product to the deleted stack
                pushStack(&deletedStack, current);

                // Avoid freeing the memory as the stack still references the product
                return;
            }
            return;
        }
...
int main() {
    ...
    while (1) {
        ...
        switch (choice) {
            ...
            case 7:
                printf("Enter From Category Number: ");
                scanf("%d", &fromCategory);
                printf("Enter To Category Number: ");
                scanf("%d", &toCategory);
                addEdge(categoryGraph, fromCategory, toCategory);
                break;

            case 8:
                printf("Enter Starting Category Number: ");
                scanf("%d", &fromCategory);
                bfs(categoryGraph, fromCategory, 106);
                break;
